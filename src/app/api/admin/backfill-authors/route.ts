...
export async function GET() {
  ...
  if (!books?.length) return NextResponse.json({ message: "No books found" })
  ...
  for (const book of books) {
    try {
      ...
      for (let i = 0; i < authorNames.length; i++) {
        ...
        // INSERT … ON CONFLICT (name) DO UPDATE to get the id reliably
        const { data: author, error: authorErr } = await admin
          .from("authors")
          .upsert({ name, sort_name }, { onConflict: "name" })
          .select("id")
          .single()

        if (authorErr || !author) {
          errors.push(`author upsert failed for "${name}": ${authorErr?.message}`)
          continue
        }
